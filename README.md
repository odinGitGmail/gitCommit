...
3. 选择“提交代码”或“标记管理”
4. 点击 Git Flow 按钮（分支图标）进行 Git Flow 操作
5. 或者使用命令面板（`Ctrl+Shift+P` / `Cmd+Shift+P`），输入相关命令

> **注意**：选择“提交代码”后，扩展会检查是否存在 `.gitcommit` 配置文件。如果不存在，会询问是否创建；标记管理不依赖该配置文件。

### Commit 流程

1. **选择操作**：点击 SCM 面板中的扩展图标，然后选择“提交代码”
2. **选择模板**：选择提交使用的模板（默认 Angular 格式）
3. **选择类型**：选择提交类型（feat、fix、docs 等）
4. **填写信息**：
   - Scope（可选）：修改范围
   - Subject（必填）：提交概述
   - Body（可选）：详细说明
   - Footer（可选）：备注信息
5. **完成**：提交信息会自动填充到 Git 提交输入框，如果启用了 `autoVersion`，会自动添加版本信息
...
1. 选择“附注标记”或“轻量标记”
2. 选择标记目标：当前分支最新提交（`HEAD`）或某个本地分支的最新提交
3. 输入符合 Git 命名规则的标记名称，例如 `v1.0.0`
4. 创建附注标记时，继续输入标记说明
...
2. 扩展实时获取该远程仓库中的全部标记
3. 选择一个或多个远程标记
4. 确认后仅删除所选远程标记，不会同时删除本地标记

#### 推送全部标记

1. 选择目标远程仓库
2. 扩展获取远程标记，并显示本地标记总数及远程尚不存在的标记数量
3. 确认后推送全部本地标记
